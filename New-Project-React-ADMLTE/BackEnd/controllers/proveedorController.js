...
exports.buscarProveedores = async(req,res) => {
    try {
       ...
       res.json({proveedores});
        ...
        console.log(error)
        res.status(500).send({msg:"Hubo un error al buscar los proveedores"});
    }
};
...
exports.agregarProveedores = async(req, res) => {
    try {
        ...
        res.send({msg:"El proveedor fue agregado"});
    } catch (error) {
        console.log(error)
        res.status(500).send({msg:"Hubo un error al agregar el proveedor"});
    }
};
...
exports.buscarProveedor = async(req,res) => {
    try {
       ...
       res.json(proveedor);
        ...
        console.log(error)
        res.status(404).send({msg:"Hubo un error al buscar el proveedor"});
    }
};
...
exports.eliminarProveedor = async (req, res) => {
    try {
        ...
        console.log(error)
        res.status(500).send({msg:"Hubo un error al eliminar el proveedor"});
    }
};
...
exports.actualizarProveedor = async (req, res) => {
    try {
      ...
      if (!proveedor) {res.status(404).send({msg:"Proveedor no encontrado"});
      }else {
        res.json({msg:"Proveedor actualizado correctamente"});
      }
    } catch (error) {
      console.log(error);
      res.status(500).send({msg:"Hubo un error al actualizar el proveedor"});
    }
  }; 